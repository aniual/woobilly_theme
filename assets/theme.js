(function ($) {
    ...
    var halo = {
        ...
        menuSidebarMobileToggle: function() {
            // 点击父菜单项，显示/隐藏子菜单
            $body.on('click', '.site-nav-mobile .list-menu .menu_lv_1__action', function (e) {
                e.preventDefault();
                e.stopPropagation();
                
                var $target = $(this);
                var $submenu = $target.next('.header__submenu');
                
                // 如果该菜单项已经展开，收起子菜单
                if ($submenu.hasClass('is-open')) {
                    $submenu.removeClass('is-open').addClass('is-hidden');
                } else {
                    // 展开子菜单
                    $submenu.removeClass('is-hidden').addClass('is-open');
                    // 收起其他同级菜单的子菜单
                    $target.parent().siblings().find('.header__submenu').removeClass('is-open').addClass('is-hidden');
                }
            });
        
            // 点击子菜单项时，导航到子页面
            $body.on('click', '.site-nav-mobile .list-menu .menu_lv_2__action', function (e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            // 点击页面其他区域时，关闭所有展开的子菜单
            $(document).on('click', function (e) {
                if (!$(e.target).closest('.site-nav-mobile').length) {
                    $('.site-nav-mobile .header__submenu').removeClass('is-open').addClass('is-hidden');
                }
            });
        };
